...
def reset_system():
    ...
    for key in ['documents', 'vectorstore', 'retriever', 'llm', 'eval_results']:
...
def show_evaluation_interface():
    ...
    # Add a container for the run evaluation button and results
    eval_container = st.container()
    
    with eval_container:
        if st.button("🧪 Run Evaluation", type="primary", key="run_eval_button"):
            # Clear previous results when running new evaluation
...
Answer:"""
            ...
            # Store results with unique timestamp
...
def display_evaluation_results():
    ...
    eval_timestamp = st.session_state.eval_results.get('timestamp', datetime.now().isoformat())
    ...
    chart_key = f"eval_metrics_chart_{eval_timestamp.replace(':', '_').replace('-', '_').replace('.', '_')}"
...
def show_analytics():
    ...
    with col1:
        ...
        st.plotly_chart(fig_time, use_container_width=True, key=f"response_time_line_{current_time}")
    ...
    with col2:
        ...
        st.plotly_chart(fig_hist, use_container_width=True, key=f"response_time_hist_{current_time}")
    ...
    if all_sources:
        ...
        st.plotly_chart(fig_sources, use_container_width=True, key=f"doc_usage_{current_time}")
    ...
    if len(st.session_state.qa_history) > 1:
        ...
        st.plotly_chart(fig_trend, use_container_width=True, key=f"perf_trend_{current_time}")
